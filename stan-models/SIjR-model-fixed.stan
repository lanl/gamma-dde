 ...
 * This version uses the "fixed" hypoxponential approximation.
 */
...
functions {
     ...
     * the rate through the state is a constant n/tau, cf. Erlang.
     */
    vector ode(real t, vector y, real beta, real j, real tau) {
    ...
    real generation_interval_lpdf(real T, real alpha, real beta) {
        return gamma_lccdf(T | alpha, beta) + log(beta/alpha);
    }
...
parameters {
    ...
    real<lower=0, upper=1> eps; // initial fractio of infected individuals
}
...
model {    
    ...
    for ( n in 1:K ) { 
        if ( j < n && n < j+1 ) {
            ...
            u0[2] = eps;
            // solve ivp
            vector[n+2] sol[N] = ode_rk45(ode, u0, 0.0, T, beta, j, tau);
...
generated quantities {
    ...
    for ( n in 1:K ) { 
        if ( j < n && n < j+1 ) {
            ...
            u0[2] = eps;
            // solve ivp
            vector[n+2] fullsol[N] = ode_rk45(ode, u0, 0.0, T, beta, j, tau);
            // simulate data
            for ( i in 1:N ) {
                real yhat;
                if ( i == 1 ) {
